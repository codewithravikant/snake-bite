...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const editNpcButton = document.getElementById('editNpcButton');
  const botCustomizationModal = document.getElementById('botCustomizationModal');
  
  // Bot configuration state
  let botConfig = null; // Will store array of {name, color, difficulty}
  ...
  function updateModeUI(selectedMode) {
    if (selectedMode === 'single-player') {
      ...
      if (editNpcButton) editNpcButton.style.display = 'block';
      joinButton.textContent = 'Start Single-Player Game';
      // Reset bot config when switching to single-player
      botConfig = null;
      ...
      if (singlePlayerGameOptions) singlePlayerGameOptions.style.display = 'block';
      if (editNpcButton) editNpcButton.style.display = 'none';
  ...
  // Bot Customization Modal Logic
  function initializeBotConfig(npcCount) {
    // Initialize bot config with defaults
    const defaultNames = ['Bot-Alpha', 'Bot-Beta', 'Bot-Gamma'];
    const defaultColors = ['#FF0000', '#00FF00', '#0000FF']; // Red, Green, Blue
    const defaultDifficulties = ['easy', 'medium', 'hard'];
    
    botConfig = [];
    for (let i = 0; i < npcCount; i++) {
      botConfig.push({
        name: defaultNames[i] || `Bot-${String.fromCharCode(65 + i)}`,
        color: defaultColors[i % defaultColors.length],
        difficulty: defaultDifficulties[i] || 'medium'
      });
    }
  }

  function openBotCustomizationModal() {
    const npcCount = parseInt(npcCountSelect.value);
    if (npcCount === 0) return;

    // Initialize config if not already set
    if (!botConfig || botConfig.length !== npcCount) {
      initializeBotConfig(npcCount);
    }
      ...
      botDiv.innerHTML = `
        ...
        <div class="bot-config-row">
          <div class="bot-config-field">
            <label>Name (must start with "Bot", max 10 chars)</label>
            <div class="bot-name-input-wrapper">
              <span class="bot-name-prefix">Bot</span>
              <input type="text" class="bot-name-input" data-bot-index="${i}" 
                     value="${bot.name.substring(3)}" maxlength="7" 
                     placeholder="Alpha" />
            </div>
          </div>
          <div class="bot-config-field">
            <label>Color</label>
            <input type="color" class="bot-color-input" data-bot-index="${i}" 
                   value="${bot.color}" />
          </div>
          <div class="bot-config-field">
            <label>Difficulty</label>
            <select class="bot-difficulty-select" data-bot-index="${i}">
              <option value="easy" ${bot.difficulty === 'easy' ? 'selected' : ''}>1 - Easy</option>
              <option value="medium" ${bot.difficulty === 'medium' ? 'selected' : ''}>2 - Medium</option>
              <option value="hard" ${bot.difficulty === 'hard' ? 'selected' : ''}>3 - Hard</option>
            </select>
          </div>
        </div>
      `;
    ...
    const errorDiv = document.getElementById('botValidationError');
    if (errorDiv) {
      errorDiv.style.display = 'none';
      errorDiv.textContent = '';
    }

    botCustomizationModal.style.display = 'flex';
  }

  function closeBotCustomizationModal() {
    botCustomizationModal.style.display = 'none';
  }

  function validateBotConfig(playerName) {
    const errors = [];
    const names = new Set();
    const npcCount = parseInt(npcCountSelect.value);

    // Collect all bot names from inputs
    for (let i = 0; i < npcCount; i++) {
      const nameInput = document.querySelector(`.bot-name-input[data-bot-index="${i}"]`);
      if (!nameInput) continue;

      const nameSuffix = nameInput.value.trim();
      const fullName = 'Bot' + nameSuffix;

      // Check if starts with "Bot" (always true since we prefix it)
      if (fullName.length < 4) {
        errors.push(`NPC ${i + 1}: Name must be at least 4 characters (including "Bot" prefix)`);
        continue;
      }

      // Check max length
      if (fullName.length > 10) {
        errors.push(`NPC ${i + 1}: Name must be at most 10 characters (currently ${fullName.length})`);
        continue;
      }

      // Check uniqueness
      if (names.has(fullName.toLowerCase())) {
        errors.push(`NPC ${i + 1}: Name "${fullName}" is already used`);
        continue;
      }
      names.add(fullName.toLowerCase());

      // Check if different from player name
      if (playerName && fullName.toLowerCase() === playerName.trim().toLowerCase()) {
        errors.push(`NPC ${i + 1}: Name cannot be the same as your player name`);
      }
    }
  ...
  function saveBotConfig() {
    const playerName = singlePlayerNameInput ? singlePlayerNameInput.value.trim() : '';
    const errors = validateBotConfig(playerName);

    if (errors.length > 0) {
      const errorDiv = document.getElementById('botValidationError');
      if (errorDiv) {
        errorDiv.innerHTML = '<strong>Validation Errors:</strong><ul style="margin: 5px 0; padding-left: 20px;">' +
    ...
    for (let i = 0; i < npcCount; i++) {
      const nameInput = document.querySelector(`.bot-name-input[data-bot-index="${i}"]`);
      const colorInput = document.querySelector(`.bot-color-input[data-bot-index="${i}"]`);
      const difficultySelect = document.querySelector(`.bot-difficulty-select[data-bot-index="${i}"]`);

      if (nameInput && colorInput && difficultySelect) {
        const nameSuffix = nameInput.value.trim();
        const fullName = 'Bot' + nameSuffix;
    ...
    closeBotCustomizationModal();
  }

  // Event listeners for bot customization modal
  if (editNpcButton) {
    editNpcButton.addEventListener('click', () => {
      const selectedMode = document.querySelector('input[name="gameMode"]:checked')?.value;
      if (selectedMode === 'single-player') {
        openBotCustomizationModal();
      }
    });
  }

  if (botCustomizationModal) {
    const closeBtn = botCustomizationModal.querySelector('.bot-modal-close');
    const cancelBtn = botCustomizationModal.querySelector('.bot-modal-cancel');
    const saveBtn = botCustomizationModal.querySelector('.bot-modal-save');

    if (closeBtn) {
      closeBtn.addEventListener('click', closeBotCustomizationModal);
    }
    if (cancelBtn) {
      cancelBtn.addEventListener('click', closeBotCustomizationModal);
    }
    if (saveBtn) {
    ...
    botCustomizationModal.addEventListener('click', (e) => {
      if (e.target === botCustomizationModal) {
        closeBotCustomizationModal();
      }
    });
  }

  // Reset bot config when NPC count changes
  if (npcCountSelect) {
    npcCountSelect.addEventListener('change', () => {
      botConfig = null; // Reset so it will be reinitialized with new count
    });
  }
  ...
  if (joinButton) {
    ...
    joinButton.addEventListener('click', (e) => {
      ...
      if (selectedMode === 'single-player' || selectedMode === 'solo') {
        ...
        // Get game options for single-player/solo
        const wallModeToggle = document.getElementById('singlePlayerWallModeToggle');
        const strictModeToggle = document.getElementById('singlePlayerStrictModeToggle');
        const wallMode = wallModeToggle ? wallModeToggle.checked : false;
        const strictMode = strictModeToggle ? strictModeToggle.checked : false;
        
        // Initialize bot config if not already set (use defaults)
        if (selectedMode === 'single-player' && npcCount > 0 && (!botConfig || botConfig.length !== npcCount)) {
          initializeBotConfig(npcCount);
        }
        
        console.log('Emitting startSinglePlayer event with options:', { playerName, npcCount, selectedMode, controlScheme, wallMode, strictMode, botConfig });
        ...
        const gameInitData = {
          ...
          botConfig: selectedMode === 'single-player' && npcCount > 0 ? botConfig : null,
    ...
    function renderSessionHistory(sessions) {
      ...
      sessions.forEach(session => {
        ...
        const durationLabel = durationSeconds !== null
          ...
          : '--';
        const winnerLabel = session.winnerName
          ? `${session.winnerName}${session.winnerScore !== null ? ` (${session.winnerScore})` : ''}`
          : 'In progress';
