...
- **Welcome Screen**: Interactive welcome screen with animated snake that appears on every page load/hard refresh
...
- **Single-Player Mode (vs NPCs)**: Play against 1-3 AI opponents with intelligent NPC behavior
  - **NPC Customization**: Customize bot names (must start with "Bot"), colors, and difficulties
  - **NPC Difficulty Levels**: Easy (human-like, makes mistakes), Medium (competitive but beatable), Hard (nearly unbeatable)
  - **Dynamic Difficulty**: NPCs automatically scale difficulty based on snake length (Easy→Medium at length 20, Medium→Hard at length 30)
  - **Color Conflict Prevention**: Bots automatically avoid using the same color as the human player
  - **Full Collision**: NPCs can collide with human players (no immunity) - feels like multiplayer
...
- **Wall Mode**: Optional wall collision mode (walls kill instead of wrapping)
  - Visual indicator: Red glowing border appears around game board when active
- **Strict Mode**: Collision detection options (all body collisions fatal vs only head collisions)
  - Available in all game modes (Multi-Player, Single-Player, and Solo)
...
│   │   └── bot-customization.css # Bot customization modal styles
│   ├── js/
...
│   │   └── welcome.js        # Welcome screen logic and animation
│   └── assets/
...
5. (Optional) Click "Edit NPC" to customize bot names, colors, and difficulties
   - Bot names must start with "Bot" (3 letters fixed) and be max 10 characters total
   - Choose custom colors for each bot (color picker)
   - Select difficulty: 1=Easy, 2=Medium, 3=Hard
   - Maximum 3 bots can be configured
6. Choose game options (Wall Mode, Strict Mode, Time Limit)
...
- **Resizable**: Drag the resize handle (bottom-right corner) to adjust chat window size
- **Minimizable**: Click the minimize button to collapse chat to header only
- Chat window is half the default size with slightly reduced opacity for better visibility
...
- Button size reduced by 30% for better UI balance

### Welcome Screen

- **Interactive Animation**: Animated snake that moves, eats food, and makes eye contact
- **Sound Effects**: Audio feedback for eating food and clicking buttons
- **Appears on Every Load**: Shows on every page load/hard refresh (no localStorage tracking)
- **Sound Toggle**: Mute/unmute button for welcome screen sounds
- **Smooth Transitions**: Fade in/out animations for better user experience
...
- **NPC AI System**: Intelligent bot behavior with three difficulty levels
  - Easy: Human-like movement, high error rate, no pathfinding
  - Medium: BFS pathfinding, limited space checking, low error rate
  - Hard: Perfect pathfinding, flood fill safety, tail-chasing strategy, zero errors
- **Color Management**: Automatic color conflict prevention between human players and bots
