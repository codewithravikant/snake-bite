...
function createGameState(players, enablePowerups = false, wallMode = false, timeLimit = null, strictMode = false) {
  ...
  // Separate human players and NPCs
  const humanPlayers = players.filter(p => (p.type || 'human') === 'human');
  const npcPlayers = players.filter(p => (p.type || 'human') === 'npc');
  
  // Track used colors to avoid conflicts
  const usedColors = new Set();
  
  // First, assign colors to human players (they get priority)
  ...
  // Then, assign colors to NPCs (avoiding human player colors)
  npcPlayers.forEach((player, npcIndex) => {
    const posIndex = humanPlayers.length + npcIndex;
    const pos = startPositions[posIndex % startPositions.length];
    
    // Use custom color if provided, but check if it conflicts with human player
    let playerColor;
    if (player.color) {
      // If custom color conflicts with human player's color, use alternative
      if (usedColors.has(player.color)) {
        // Find first available color from palette that's not used
        playerColor = colorPalette.find(color => !usedColors.has(color)) || colorPalette[(humanPlayers.length + npcIndex) % colorPalette.length];
        console.log(`[gameLogic] NPC ${player.name} had conflicting color ${player.color}, using ${playerColor} instead`);
      } else {
        playerColor = player.color;
      }
    } else {
      // Use default palette, but skip colors already used by human players
      const availableColors = colorPalette.filter(color => !usedColors.has(color));
      if (availableColors.length > 0) {
        playerColor = availableColors[npcIndex % availableColors.length];
      } else {
        // Fallback: use palette with offset (shouldn't happen with 4 colors and max 3 NPCs)
        playerColor = colorPalette[(humanPlayers.length + npcIndex) % colorPalette.length];
      }
    }
