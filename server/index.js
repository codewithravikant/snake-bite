...
io.on('connection', (socket) => {
  ...
  socket.on('startSinglePlayer', (data) => {
    ...
    const strictMode = gameOptions && gameOptions.strictMode !== undefined ? Boolean(gameOptions.strictMode) : false;
    ...
    const room = {
      ...
      gameOptions: {
        ...
        strictMode: strictMode,    // From client or default: false
    ...
    if (actualGameMode === 'single-player' && npcCount > 0) {
      // Default bot configuration (fallback if botConfig not provided)
      const defaultNpcNames = ['Bot-Alpha', 'Bot-Beta', 'Bot-Gamma'];
      const defaultColors = ['#FF0000', '#00FF00', '#0000FF']; // Red, Green, Blue
      const defaultDifficulties = ['easy', 'medium', 'hard'];
      
      // Use provided botConfig if available and valid, otherwise use defaults
      const botConfig = data.botConfig;
      let useCustomConfig = false;
      
      if (botConfig && Array.isArray(botConfig) && botConfig.length === npcCount) {
        // Validate botConfig
        const isValid = botConfig.every((bot, index) => {
          return bot && 
                 typeof bot.name === 'string' && bot.name.length >= 4 && bot.name.length <= 10 &&
                 bot.name.toLowerCase().startsWith('bot') &&
                 typeof bot.color === 'string' && /^#[0-9A-Fa-f]{6}$/.test(bot.color) &&
                 ['easy', 'medium', 'hard'].includes(bot.difficulty);
        });
        
        if (isValid) {
          useCustomConfig = true;
        } else {
          devLog.error('startSinglePlayer: Invalid botConfig provided, using defaults');
        }
      }
      
      for (let i = 0; i < npcCount; i++) {
        const npcId = `npc-${roomCode}-${i}`;
        const npcName = useCustomConfig ? botConfig[i].name : defaultNpcNames[i];
        const npcColor = useCustomConfig ? botConfig[i].color : defaultColors[i % defaultColors.length];
        const difficulty = useCustomConfig ? botConfig[i].difficulty : (defaultDifficulties[i] || 'medium');
        ...
        room.players.set(npcId, {
          ...
          type: 'npc',
          isHost: false,
          color: npcColor // Store custom color for use in createGameState
        });
    ...
    for (const [roomCode, room] of rooms.entries()) {
      ...
      if (room.sessionId && gameSessions.has(room.sessionId)) {
        ...
        if (!session.endTime) {
          ...
          devLog.log(`[SESSION ENDED] Session ${room.sessionId} ended on disconnect (page refresh)`, {
            roomCode,
            playerName: player.name
          });
        }
      ...
      if (room.isGameActive) {
        ...
        // For solo/single-player modes: end game immediately when player disconnects
        if (room.gameMode === 'solo' || room.gameMode === 'single-player') {
          console.log(`Player ${player.name} disconnected from ${room.gameMode} game, ending game immediately`);
          ...
          if (room.gameState && room.gameState.players[player.id]) {
            ...
            devLog.log(`Marked player ${player.name} (${player.id}) as dead on disconnect`);
          }
          ...
          // Mark session as ended and clean up room immediately
          markSessionAsEnded(roomCode, 'player_disconnected');
          removePublicRoom(roomCode);
          
          // Clean up room immediately (no timeout for solo/single-player)
          setTimeout(() => {
            const checkRoom = rooms.get(roomCode);
            if (checkRoom && !checkRoom.isGameActive) {
              rooms.delete(roomCode);
              console.log(`Room ${roomCode} deleted after solo/single-player disconnect`);
            }
          }, 1000);
